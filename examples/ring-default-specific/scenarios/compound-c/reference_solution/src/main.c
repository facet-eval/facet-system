...
static int append_bytes(const char *path, const uint8_t *bytes, size_t n) {
    ...
    if (!f) return -1;
    if (fwrite(bytes, 1, n, f) != n) { fclose(f); return -1; }
    fclose(f);
    return 0;
}
...
static int cmd_dump(const char *path) {
    ...
    tlv_status hs = tlv_validate_header(buf, size);
    if (hs != TLV_OK) {
        fprintf(stderr, "tlv: invalid header (%d)\n", (int)hs);
        free(buf);
        return 1;
    }
    ...
    tlv_status ps = tlv_parse_buffer(buf, size, &list);
    if (ps != TLV_OK) {
...
static int cmd_append(int argc, char **argv) {
    ...
    if (!file_exists(path)) {
        ...
        if (read_whole_file(path, &buf, &size) == 0) {
            ...
            free(buf);
            if (hs != TLV_OK) {
                fprintf(stderr, "tlv: existing file has invalid header (%d)\n", (int)hs);
                return 1;
            }
        }
    }
    ...
    uint8_t bytes[1024];
    size_t   n_bytes = 0;
    tlv_status es = TLV_OK;
    if (strcmp(type_str, "int32") == 0) {
        long v = strtol(value_str, NULL, 10);
        es = tlv_encode_int32((int32_t)v, bytes, sizeof(bytes), &n_bytes);
    } else if (strcmp(type_str, "string") == 0) {
        es = tlv_encode_string(value_str, strlen(value_str), bytes, sizeof(bytes), &n_bytes);
    } else if (strcmp(type_str, "bool") == 0) {
        int v;
        if (strcmp(value_str, "true") == 0) v = 1;
        else if (strcmp(value_str, "false") == 0) v = 0;
        else {
            fprintf(stderr, "tlv: --value=true|false required for bool\n");
            return 1;
        }
        es = tlv_encode_bool(v, bytes, sizeof(bytes), &n_bytes);
    } else {
        fprintf(stderr, "tlv append: unknown --type=%s\n", type_str);
        return 1;
    }
    if (es != TLV_OK) {
        fprintf(stderr, "tlv append: encode failed (%d)\n", (int)es);
        return 1;
    }
